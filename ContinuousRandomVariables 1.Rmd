...
mean((simulatedData_Male-mean(simulatedData_Male))^2)#Mean of the squared deviation around the deviation is the variance
sqrt(mean((simulatedData_Male-mean(simulatedData_Male))^2))#Standard Deviation
```
```{r}
mean(simulatedData_Male)
...
```{r}
x=seq(138,202,0.1)
#y=(1/sqrt(2*pi*8^2))*exp(-(x-170)^2/2*8^2)
f_x=function(x){
  return((1/sqrt(2*pi*8^2))*exp(-(x-170)^2/8^2))
}
...
p = ggplot(data = df) +
  ...
  colour = 'blue') +labs(x='Height(cm)',y='Density',title="Continuous Random Variable Distribution")
p
```
```{r}
x=seq(138,202,0.1)
...
f2_x=function(x){
  return((1/sqrt(2*pi*(4^2)))*exp(-(x-190)^2/(2*(4^2))))
}
y1=f1_x(x)
y2=f2_x(x)
...
df
colnames(df)=c('x','f1_x','f2_x')
df
p = ggplot(data = df) +
  geom_line(aes(x = x, y = f1_x,colour = 'Male Students'))+
  geom_line(aes(x = x,y=f2_x,color='Unknown'))+
  scale_color_manual(name='Students',values=c('Male Students'='red','Unknown'='blue'))+
  labs(x="Height(cm)",y="Density")
...
x=seq(138,202,0.1)
f1_x=function(x){
  return (dnorm(x,170,8))
}
f2_x=function(x){
  return (dnorm(x,190,4))
}
y1=f1_x(x)
y2=f2_x(x)
...
p = ggplot(data = df) +
  geom_line(aes(x = x, y = f1_x,colour = 'Male Students'))+
  geom_line(aes(x = x,y=f2_x,color='Unknown'))+
  scale_color_manual(name='Students',values=c('Male Students'='green','Unknown'='chocolate'))+
  labs(x="Height(cm)",y="Density")
...
```{r}
dnorm(190,170,8)#Density
pnorm(190,170,8)#Actual probability P(x<=190)
pnorm(190,170,8)-pnorm(185,170,8)#Probability of students height in between 185 and 190
pnorm(190,170,8,lower.tail = FALSE)#Actual probability P(x>=190)
```
