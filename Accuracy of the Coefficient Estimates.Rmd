...
```{r}
model1 = lm(data = carData, mpg ~ wt)
summary(model1)
residuals(model1)
predict(model1)
coef(model1)
summary(model1)$coefficients
summary(model1)$coefficients['wt', 'Std. Error']
coef(model1)['wt']-2*summary(model1)$coefficients['wt','Std. Error'] 
coef(model1)['wt']+2*summary(model1)$coefficients['wt','Std. Error']
sum(residuals(model1)^2)
carData$mpg - mean(carData$mpg) #mean centered mpg
carData$mpg - mean(carData$mpg)/sd(carData$mpg)
(carData$mpg - mean(carData$mpg))^2
mean((carData$mpg - mean(carData$mpg))^2)
TSS =sum((carData$mpg - mean(carData$mpg))^2)
TSS
